...
IndiceIdentificadores::IndiceIdentificadores(std::string path){
...
int IndiceIdentificadores::agregar(unsigned int sourcePosition, std::string titulo){
    unsigned int tituloId = this->obtenerId(titulo);
    AutorOcurrencia* ocur = new AutorOcurrencia(tituloId, sourcePosition);
    this->guardarOcurrencia(ocur);
    delete ocur;
    return 0;
}

unsigned int IndiceIdentificadores::obtenerId(std::string titulo){
    std::ifstream file;
    std::string p;
    int found = 0;
    unsigned int id =0;
    ...
    if(!found){
        std::ofstream file2;
        file2.open(identificadores.c_str(), std::fstream::app);
        id = file2.tellp();
...
int IndiceIdentificadores::buscarId(std::string titulo){
    std::ifstream file;
    std::string p;
    int found = 0;
    unsigned int id =0;
...
int IndiceIdentificadores::guardarOcurrencia(AutorOcurrencia* ocur){
    std::ofstream file;
...
int IndiceIdentificadores::pack(){
...
int IndiceIdentificadores::packAppend(){
    if(Utils::existeArchivo(temporalOcurrencias)){
        std::cout << "Indexando identificadores...";
        std::cout.flush();
        SortExterno<AutorOcurrencia>* sort = new SortExterno<AutorOcurrencia>(this->temporalOcurrencias,4096);
...
int IndiceIdentificadores::copylist(unsigned int reflista,std::list<unsigned int>*lista){
    std::ifstream file;
...
unsigned int IndiceIdentificadores::getRefLista(){
    std::ofstream file;
...
int IndiceIdentificadores::guardarRefLista(std::list<unsigned int>* lista){
...
int IndiceIdentificadores::printOcurrencias(){
    std::ifstream file;
...
int IndiceIdentificadores::recuperarPlus(std::string titulo,std::list<unsigned int> *lista){
...
int IndiceIdentificadores::recuperar(std::string titulo, std::list<unsigned int> *lista){
...
int IndiceIdentificadores::eliminarTodo(){
