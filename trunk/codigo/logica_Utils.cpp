...
void Utils::dividirRss(string fileName, int contador, int fecha) {
    string line = "";
    ifstream inFile;
    string filePathName = "fuentesRss/" + fileName;
    ...
    while (!inFile.eof()) {
      ...
      salidaStr = "../fuentes/rss_" +  IntToStr(contador) + "_" + IntToStr(i) +".txt";
...
void Utils::dividirRssFiles (int fecha) {
  DIR *dir;
  struct dirent *ent;
  if ((dir = opendir ("fuentesRss")) != NULL) {
    readdir (dir);
    readdir (dir);
    string comp;
    int contador = 0;
...
void Utils::dividirTwt(string fileName, int contador, int fecha) {
    string line = "";
    ifstream inFile;
    string filePathName = "fuentesTwt/" + fileName;
    inFile.open(filePathName.c_str());
    ...
    getline(inFile,line);
    while (!inFile.eof() && line.compare(marcador)!=0) {
      salidaStr = "../fuentes/twt_" +  IntToStr(contador) + "_" + IntToStr(i) +".txt";
      salida.open(salidaStr.c_str(), ios::trunc);
...
void Utils::dividirTwtFiles (int fecha) {
  DIR *dir;
  struct dirent *ent;
  if ((dir = opendir ("fuentesTwt")) != NULL) {
    readdir (dir);
    int contador = 0;
    string comp;
    while ((ent = readdir (dir)) != NULL) {
